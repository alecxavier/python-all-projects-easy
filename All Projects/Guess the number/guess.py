...
# letters = [random.choice([chr(random.randint(65, 90)), chr(random.randint(97, 122))]) for x in range(0, 8)]
# print(letters)

# sample = chr(random.randint(65, 90))
# print(sample)


letters = [random.choice([chr(random.randint(97, 122)), chr(random.randint(65, 90))]) for d in range(0, 8)]
numbers = [str(random.randint(0, 9)) for x in range(0, 4)]
values = letters + numbers

random.shuffle(values)
password = ''.join(values)
generated_password = f'Your new password is: {password}'
...
def play_guess(x):
    generated_number = random.randint(1, x)
    guess = 0
    while guess != generated_number:
        guess = int(input(f'Pick a number from 1 - {x}:  '))    
        if guess > generated_number:
            print(f"{guess} is incorrect! You might want to go lower! ")
        elif guess < generated_number:
            print(f"{guess} is incorrect! You might want to go higher! ")

    print(f"Yay! Congrats! You have guessed the number {generated_number}")
...
def game():
    ...
    result_from_toss = ''
    while result_from_toss != coin_toss[1]:
        result_from_toss = random.choice(coin_toss)
        if result_from_toss == 'Heads':
            heads = f'You got {result_from_toss} bro'
            print(heads)
        else: 
            continue

    result = f"Wow, you're a strong, independent man! Here ya go! {result_from_toss}"
    print(result)
...
def hangman_game(guess_letter): 
    ...
    # to this, this function converts the chosen word from a string to a list of individual characters.


    guess = guess_letter
    #the string passed into the function's parameter is being held by the guess variable.

    guessed_word = []
    while guessed_word != chosen_word_list:


        guess_a_new_word = input("Pick a letter again!: ")
        guess = guess_a_new_word

        if guess in chosen_word_list:
            guessed_word.append(guess)
            print(f'You have guessed a letter inside the chosen word! {guessed_word} ')

        else: 
            print('You guessed nothing! ')
...
uploaded_video = datetime.date(2017, 5, 4)
print(uploaded_video.strftime('%A'))
...
# print(chosen_word_list)
    ...
    # letters = chr(65, 90), chr(97, 122)
    
    # def hangman():
    #     if guess in chosen_word:
    #         letters_left = f"Correct! You have {word_length - 1} letters left"
    #         print(letters_left)
    #     else:
    #         pass
...
# numbers = [str(random.randint(1,9)) for x in range(0,3)]
